...
$testRoot = Join-Path $tempBase ('wechat-article-api-offline-' + [Guid]::NewGuid().ToString('N'))
$oldCodexHome = $env:CODEX_HOME
$firstLock = $null

try {
    & (Join-Path $PSScriptRoot 'New-WeChatArticleExample.ps1') -OutputFolder $testRoot | Out-Null
    $validated = & (Join-Path $PSScriptRoot 'Test-WeChatArticlePackage.ps1') -InputFolder $testRoot -PassThru
    ...
    $manifestPath = Join-Path $testRoot 'article.json'
    $originalManifest = Get-Content -LiteralPath $manifestPath -Raw -Encoding utf8
    $invalidManifest = $originalManifest | ConvertFrom-Json -AsHashtable
    $invalidManifest.contentSourceUrl = 'javascript:alert(1)'
    Write-JsonNoBom -Path $manifestPath -Value $invalidManifest
    $sourceUrlRejected = $false
    try {
        & (Join-Path $PSScriptRoot 'Test-WeChatArticlePackage.ps1') -InputFolder $testRoot -PassThru | Out-Null
    }
    catch {
        $sourceUrlRejected = $_.Exception.Message -match 'contentSourceUrl'
    }
    Assert-True $sourceUrlRejected '非法 contentSourceUrl 没有明确拒绝。'
    ...
    try {
        & (Join-Path $PSScriptRoot 'Publish-WeChatArticleDraft.ps1') -InputFolder $testRoot -CredentialFile $fakeCredentialPath | Out-Null
    }
    catch {
        $accountRejected = $_.Exception.Message -match '素材包账号为 .+，凭据账号为 .+，已停止'
    }
    ...
    $matchingCredentialPath = Join-Path $testRoot 'matching-credential.dpapi.json'
    Write-JsonNoBom -Path $matchingCredentialPath -Value ([ordered]@{
        schemaVersion = 1
        appId = 'wx' + ('0' * 16)
        accountName = 'YOUR_WECHAT_ACCOUNT_NAME'
        appSecretProtected = Protect-WeChatSecret -PlainText ('offline-test-' + [Guid]::NewGuid().ToString('N'))
    })
    Write-JsonNoBom -Path (Join-Path $testRoot 'wechat-article-api-state.json') -Value ([ordered]@{
        schemaVersion = 1
        type = 'news'
        fingerprint = $validated.Fingerprint
        stage = 'draft_submitting'
        bodyImageUrls = [ordered]@{}
        coverMediaId = 'offline-cover'
        draftMediaId = $null
        submissionStartedAt = [DateTimeOffset]::Now.ToString('o')
        createdAt = [DateTimeOffset]::Now.ToString('o')
        updatedAt = [DateTimeOffset]::Now.ToString('o')
    })
    $uncertainRejected = $false
    try {
        & (Join-Path $PSScriptRoot 'Publish-WeChatArticleDraft.ps1') -InputFolder $testRoot -CredentialFile $matchingCredentialPath | Out-Null
    }
    catch {
        $uncertainRejected = $_.Exception.Message -match '当前状态不确定' -and $_.Exception.Message -match '禁止自动重试'
    }
    Assert-True $uncertainRejected 'draft/add 不确定态没有在 API 调用前禁止自动重试。'

    [pscustomobject]@{
        Status = 'PASS'
        Checks = 9
        ApiCalls = 0
        Message = '文章 Skill 离线自检通过。'
    } | Format-List
}
finally {
    ...
    $safePrefix = $tempBase + [IO.Path]::DirectorySeparatorChar + 'wechat-article-api-offline-'
